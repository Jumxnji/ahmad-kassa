...
import {
  addEducationItemAction,
  removeEducationItemAction,
  updateEducationItemAction,
} from "@/actions/admin/about.actions";
import { educationItemFormSchema, type EducationItemFormInput } from "@/validators/about.validator";
...
function ItemDialog({
  ...
  const form = useForm<EducationItemFormInput>({
  ...
  function onSubmit(values: EducationItemFormInput) {
    startTransition(async () => {
      const result = item
        ? await updateEducationItemAction(item.id, values)
        : await addEducationItemAction(values);
      if (result.success) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{item ? "Edit education entry" : "New education entry"}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          ...
          <li key={item.id} className="flex items-start gap-3 rounded-lg border border-border p-4">
            ...
            <div className="flex shrink-0 items-center gap-1">
              ...
              <ConfirmDialog
                ...
                description="This education entry will be removed from the About page."
                onConfirm={async () => {
                  const result = await removeEducationItemAction(item.id);
                  if (result.success) {
                    toast.success(result.message);
                    router.refresh();
                  } else {
